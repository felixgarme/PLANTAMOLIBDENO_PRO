// 1. Comprobar si los botones ya fueron creados (VERSIÓN C)
if (!window.botonesVCCreados) { // <-- CAMBIO

    // --- A. INYECTAR CSS PARA LOS BOTONES (VERSIÓN C) ---
    // (Solo se ejecuta una vez)
    const cssId = 'v3d-pro-button-styles-VC'; // <-- CAMBIO: ID de estilo único
    if (!document.getElementById(cssId)) {
        ...
        style.innerHTML = `
            ...
            /* Contenedor de los botones (VERSIÓN C) */
            #miContenedorBotonesVC { /* <-- CAMBIO */
                position: absolute;
                top: 50vh; 
                left: 40px; 
                z-index: 100;
            ...
            /* --- BOTÓN 'SIGUIENTE' (FLECHA) (VERSIÓN C) --- */
            #v3d-next-button-VC { /* <-- CAMBIO */
                position: absolute;
                bottom: 20px;
                right: 20px;
                z-index: 101;
            ...
            #v3d-next-button-VC:hover { /* <-- CAMBIO */
                transform: scale(1.1);
                box-shadow: 0 6px 16px rgba(0, 0, 0, 0.25);
            }
            #v3d-next-button-VC::after { /* <-- CAMBIO */
                content: '›';
                font-size: 36px;
                font-weight: bold;
                color: #333;
    ...
    // --- B. CONFIGURACIÓN (VERSIÓN C) ---
    window.nivelDesbloqueadoVC = 0; // <-- CAMBIO
    ...
    const contenedorId = 'miContenedorBotonesVC'; // <-- CAMBIO
    ...
    // --- D. CREAR CADA BOTÓN ---
    botonesConfigVC.forEach((item, index) => { // <-- CAMBIO
        ...
        if (index > window.nivelDesbloqueadoVC) { // <-- CAMBIO
            btn.classList.add('disabled');
        }

        // === Evento click (apunta a funciones VC) ===
        btn.addEventListener('click', function() {
            ...
            if (v3d.puzzles && v3d.puzzles.procedures && typeof v3d.puzzles.procedures[nombreProc] === 'function') {
                ...
                // --- CAMBIO: Llaman a las funciones 'VC' ---
                window.desbloquearSiguienteNivelVC(clickedIndex);
                window.actualizarEstadoBotonesVC(clickedIndex);
    ...
    nextButton.id = 'v3d-next-button-VC'; // <-- CAMBIO
    nextButton.addEventListener('click', function() {
        const allButtons = document.querySelectorAll(`#${contenedorId} .v3d-pro-button`);
        
        const targetButton = allButtons[window.nivelDesbloqueadoVC]; // <-- CAMBIO
    ...
    // --- G. NUEVAS FUNCIONES DE CONTROL GLOBAL (VERSIÓN C) ---

    // === CAMBIO: 'actualizarEstadoBotonesVC' ===
    window.actualizarEstadoBotonesVC = function(centroIndex) {
        ...
        if (centroIndex === undefined) {
            centroIndex = window.nivelDesbloqueadoVC; // <-- CAMBIO
        }
        ...
        allButtons.forEach((btn, index) => {
            ...
            } else if (index > window.nivelDesbloqueadoVC) { // <-- CAMBIO
                btn.classList.add('disabled'); 
                allCompleted = false;
            } else {
                allCompleted = false;
        ...
        const nextBtn = document.getElementById('v3d-next-button-VC'); // <-- CAMBIO
        if (nextBtn) {
            if (allCompleted) {
    ...
    // === CAMBIO: 'desbloquearSiguienteNivelVC' ===
    window.desbloquearSiguienteNivelVC = function(indexCompletado) {
        const nuevoNivel = indexCompletado + 1;
        if (nuevoNivel > window.nivelDesbloqueadoVC) { // <-- CAMBIO
            window.nivelDesbloqueadoVC = nuevoNivel; // <-- CAMBIO
        }
    };

    // --- H. FUNCIÓN DE CONTROL GLOBAL (VERSIÓN C) ---
    // === CAMBIO: 'toggleBotonesVC' ===
    window.toggleBotonesVC = function(mostrar) {
        const cont = document.getElementById(contenedorId);
        const nextBtn = document.getElementById('v3d-next-button-VC'); // <-- CAMBIO
        ...
        if (mostrar) {
            ...
            window.actualizarEstadoBotonesVC(); // <-- CAMBIO
            ...
            setTimeout(window.actualizarEstadoBotonesVC, 500); // <-- CAMBIO
    ...
    window.toggleBotonesVC(false); // <-- CAMBIO

    // --- J. MARCAR COMO CREADOS ---
    window.botonesVCCreados = true; // <-- CAMBIO

} // <-- Fin del 'if (!window.botonesVCCreados)'